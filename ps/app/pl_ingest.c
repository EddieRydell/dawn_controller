...
#define REG_FRAME_BANK_WORDS 0x038u
#define REG_ACTIVE_BANK      0x03cu
#define REG_WRITE_BANK       0x040u
#define REG_FRAME_SEQUENCE   0x044u

#define STATUS_READY 0x00000001u
#define STATUS_OVERFLOW 0x00000002u
#define CONTROL_CLEAR_ERRORS 0x00000002u
#define FRAME_COMMIT_BANK_SHIFT 31u
#define FRAME_COMMIT_WORD_MASK 0x7fffffffu
...
void pl_ingest_snapshot(pl_ingest_snapshot_t *snapshot)
{
    ...
    snapshot->bank_words = pl_ingest_read(REG_FRAME_BANK_WORDS);
    snapshot->active_bank = pl_ingest_read(REG_ACTIVE_BANK);
    snapshot->write_bank = pl_ingest_read(REG_WRITE_BANK);
    snapshot->frame_sequence = pl_ingest_read(REG_FRAME_SEQUENCE);
...
pl_ingest_result_t pl_ingest_init(uint32_t required_words)
{
    ...
    xil_printf("pl_control=0x%08lx pl_frame=0x%08lx id=0x%08lx version=0x%08lx capacity=%lu bank_words=%lu active_bank=%lu write_bank=%lu status=0x%08lx\r\n",
...
pl_ingest_result_t pl_ingest_self_test(void)
{
    ...
    if (pl_ingest_read(REG_ACTIVE_BANK) > 1u || pl_ingest_read(REG_WRITE_BANK) > 1u) {
        return PL_INGEST_READBACK_FAILED;
    }
...
{
    uint32_t bank_words = pl_ingest_read(REG_FRAME_BANK_WORDS);
    uint32_t write_bank = pl_ingest_read(REG_WRITE_BANK);
    size_t bank_offset = (size_t)write_bank * (size_t)bank_words;
    uint32_t commit_value;

    if (write_bank > 1u || word_count > bank_words || word_count > FRAME_COMMIT_WORD_MASK) {
        return PL_INGEST_CAPACITY_TOO_SMALL;
    }
    ...
    commit_value = (write_bank << FRAME_COMMIT_BANK_SHIFT) | (uint32_t)word_count;
    pl_ingest_write(REG_FRAME_COMMIT, commit_value);
    return PL_INGEST_OK;
}
